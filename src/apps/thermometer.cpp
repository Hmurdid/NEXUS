...
static void drawGraph(float currentValue, const char* label) {
    ...
    char tempStr[16];
    snprintf(tempStr, sizeof(tempStr), "%.1fC", currentValue);
    ...
    if (historyFull || historyIndex > 0) {
        ...
        char minMax[24];
        snprintf(minMax, sizeof(minMax), "min %.1f max %.1f", minV, maxV);
...
void runThermometer() {
    ...
    while (currentState == STATE_THERMOMETER) {
